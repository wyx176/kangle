	...
	function editfrom(files)
	{
		...
		msg += '<br><br><input align="center" class="btn btn-warning" value="保存" onClick=editsave("on") type="button">';
		msg += '</div>';
	...
	function upload() 
	{
		...
		msg += '<form name="upform" enctype="multipart/form-data" action="?c=webftp&a=upsave" method="post" class="form-horizontal">';
		msg += '<p>本地文件：</p><div class="form-group"><input name="myfile" size=50 type="file" class="form-control"></div>';
		msg += '<div class="form-group"><input value="上传" type="Submit" class="btn btn-success"> &nbsp;';
		msg += ' &nbsp;<input value="清空"  type="reset" class="btn btn-default"> &nbsp;';
		msg += ' &nbsp;<input value="上传多个" type="button" onclick="show_upload_msg()"  class="btn btn-info"></div>';
		msg += '</form></div>';
