...
function piao_domain_import()
{
	var html = '<div>覆盖:<input id="replace" name="replace" type=checkbox value=1 checked></div>';
		html += '<div class="piao_div"><br>一行一个，域名(或IP)与目录之间用空格分割</div>';
		html += '<div class="piao_div"><br><textarea id="domain_import_value" rows=10 cols=60 placeholder="www.cdn.com wwwroot"></textarea>';
		html += '<div><br><input type=button class="btn btn-warning" value="导入" onclick="domain_import()"></div>';
